    ...
    <div className="bg-white rounded-lg shadow-lg p-6 h-full flex flex-col">
      ...
      <button
        onClick={() => {
          ...
          onConfirmPayment({ orderType, payments, total, notes });
        }}
        disabled={items.length === 0 || payments.length === 0 || Math.abs(remaining) > 0.01}
        className={cn(
          "mt-4 w-full py-3 rounded-lg font-semibold text-white transition-colors",
          items.length === 0 || payments.length === 0 || Math.abs(remaining) > 0.01
